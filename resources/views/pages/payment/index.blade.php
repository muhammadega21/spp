                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr>
                            ...
                            <td>{{ $item->status == 'approved' ? 'Diterima' : ($item->status == 'rejected' ? 'Ditolak' : 'Menunggu Konfirmasi') }}
                            </td>
                            <td class="flex gap-x-2">
                                ...
                                <form action="{{ route('dashboard.payment.approved', $item->id) }}" method="POST"
                                    class="confirm-with-note" data-title="Terima Pembayaran"
                                    data-text="Masukkan catatan untuk wali murid" data-icon="question">
                                    @csrf
                                    <button
                                        class="btn btn-square btn-sm btn-warning "{{ $item->status == 'approved' ? 'disabled' : '' }}><i
                                            class="bx bx-check text-base"></i></button>
                                </form>
                                <form action="{{ route('dashboard.payment.reject', $item->id) }}"method="POST"
                                    class="confirm-with-note" data-title="Tolak Pembayaran"
                                    data-text="Masukkan alasan penolakan" data-icon="warning">
                                    @csrf
                                    <button class="btn btn-square btn-sm btn-error"
                                        {{ $item->status == 'approved' ? 'disabled' : '' }}><i
                                            class="bx bx-x text-base"></i></button>
                                </form>
                            </td>
                        </tr>
